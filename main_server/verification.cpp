...
int main(int argc, char* argv[])
{
	...
	for (int row = 1; row < sheet->GetTotalRows(); row++)
	{
		...
		strcpy(p1.myposition, "원딜");
		strcpy(p2.myposition, "서폿");
		strcpy(p1.duoposition, "서폿");
		strcpy(p2.duoposition, "원딜");
		strcpy(p1.rank, "여기는 엑셀에서 랭크 정보를 가져와야됌");
		strcpy(p2.rank, "여기는 엑셀에서 랭크 정보를 가져와야됌");
		p1.clnt_sock = 0;
		p2.clnt_sock = 1;
